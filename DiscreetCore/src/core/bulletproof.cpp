...
namespace discore
{
    ...
    {
        CHECK_THROW_ERR(size + Ao <= A.size(), "Incompatible size for A");
        CHECK_THROW_ERR(size + Bo <= B.size(), "Incompatible size for B");
        CHECK_THROW_ERR(size + ao <= a.size(), "Incompatible size for a");
        CHECK_THROW_ERR(size + bo <= b.size(), "Incompatible size for b");
        CHECK_THROW_ERR(size <= maxN * maxM, "size is too large");
        CHECK_THROW_ERR(!scale || size == scale->size() / 2, "Incompatible size for scale");
        CHECK_THROW_ERR(!!extra_point == !!extra_scalar, "only one of extra point/scalar present");
    ...
    Bulletproof bulletproof_PROVE(const keyV& sv, const keyV& gamma)
    {
        ...
        for (size_t j = 0; j < M; ++j)
        {
            ...
            CHECK_THROW_ERR(test_aL == v_test, "test_aL failed");
            CHECK_THROW_ERR(test_aR == v_test, "test_aR failed");
                }
#endif
    ...
    try_again:
        ...
        for (size_t j = 0; j < M; ++j)
        {
            for (size_t i = 0; i < N; ++i)
            {
                CHECK_THROW_ERR(j + 2 < zpow.size(), "invalid zpow index");
                CHECK_THROW_ERR(i < twoN.size(), "invalid twoN index");
        ...
        {
            CHECK_THROW_ERR(j + 1 < zpow.size(), "invalid zpow index");
        ...
        CHECK_THROW_ERR(test_t == t, "test_t check failed");
#endif
        ...
        keyV R(logMN);
        int round = 0;
        keyV w(logMN); // this is the challenge x in the inner product protocol
    ...
    bool bulletproof_VERIFY(const std::vector<const Bulletproof*>& proofs)
    {
        ...
        for (const Bulletproof* p : proofs)
        {
            ...
            // check scalar range
            CHECK_ASSERT(is_reduced(proof.taux), false, "Input scalar not in range");
            CHECK_ASSERT(is_reduced(proof.mu), false, "Input scalar not in range");
            CHECK_ASSERT(is_reduced(proof.a), false, "Input scalar not in range");
            CHECK_ASSERT(is_reduced(proof.b), false, "Input scalar not in range");
            CHECK_ASSERT(is_reduced(proof.t), false, "Input scalar not in range");
            ...
            CHECK_ASSERT(!(pd.x == zero()), false, "x == 0");
            pd.x_ip = hash_cache_mash(hash_cache, pd.x, proof.taux, proof.mu, proof.t);
            CHECK_ASSERT(!(pd.x_ip == zero()), false, "x_ip == 0");
            ...
            for (size_t i = 0; i < rounds; ++i)
            {
                ...
                CHECK_ASSERT(!(pd.w[i] == zero()), false, "w[i] == 0");
            }
        ...
        for (const Bulletproof* p : proofs)
        {
            ...
            //proof8_V.resize(proof.V.size()); for (size_t i = 0; i < proof.V.size(); ++i) CHECK_THROW_ERR(ge_frombytes_vartime(&proof8_V[i], proof.V[i].bytes) == 0, "ge_frombytes_vartime failed");
            ...
            {
                CHECK_ASSERT(j + 2 < zpow.size(), false, "invalid zpow index");
            ...
            for (size_t i = 0; i < MN; ++i)
            {
                ...
                CHECK_ASSERT(2 + i / N < zpow.size(), false, "invalid zpow index");
                CHECK_ASSERT(i % N < twoN.size(), false, "invalid twoN index");
        ...
        multiexp_data.emplace_back(tmp, G);
        sc_sub(tmp.bytes, z3.bytes, y1.bytes);
        multiexp_data.emplace_back(tmp, H);
...
discore::ArgBulletproof bulletproof_prove(const uint64_t v[16], const discore::key16 gamma, uint64_t size)
{
    discore::ArgBulletproof bp = { 0 };
...
bool bulletproof_verify(discore::ArgBulletproof bp)
{
    ...
    bool test = discore::bulletproof_VERIFY(bp_);
