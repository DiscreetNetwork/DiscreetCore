...
#include "sha/sha256.h"
    ...
    bool topoint_check_order(ge_p3 *p, const unsigned char *data)
    {
        if (ge_frombytes_vartime(p, data))
            return false;
        ge_p2 r;
        ge_scalarmult(&r, curve_order().bytes, p);
        key _tmp;
        ge_tobytes(_tmp.bytes, &r);
        return _tmp == identity();
    }
    ...
    key add_keys(const keyV &a)
    {
        if (a.empty())
            return identity();
        ge_p3 p3, _tmp;
        CHECK_THROW_ERR(ge_frombytes_vartime(&p3, a[0].bytes) == 0, "ge_frombytes_vartime failed (add_keys)");
        for (size_t i = 1; i < a.size(); ++i)
        {
            CHECK_THROW_ERR(ge_frombytes_vartime(&_tmp, a[i].bytes) == 0, "ge_frombytes_vartime failed (add_keys)");
            ge_cached p2;
            ge_p3_to_cached(&p2, &_tmp);
            ge_p1p1 p1;
            ge_add(&p1, &p3, &p2);
            ge_p1p1_to_p3(&p3, &p1);
        }
    ...
    key hash_data(const ctkeyV &data)
    {
        key rv;
        if (data.empty()) {
            hash_data(rv, (const void *)"", 0);
            return rv;
        }
        hash_data(rv, &data[0], 64*data.size());
        return rv;
    }

    key hash_to_scalar(const ctkeyV &data)
    {
        key rv = hash_data(data);
    ...
    key hash_data(const keyV &data)
    {
        ...
        hash_data(rv, &data[0], data.size()*sizeof(data[0]));
        return rv;
    }
    ...
    {
        key rv;
        hash_data(rv, &data[0], 64 * sizeof(data[0]));
        return rv;
    }
    ...
    void sum_keys(key &sum, const keyV &ks)
    {
        identity(sum);
        size_t i = 0;
        for (i = 0; i < ks.size(); i++) {
            add_keys(sum, sum, ks[i]);
        }
    }
