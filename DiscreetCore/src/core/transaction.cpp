...
struct zero_commitment {
    uint64_t amount;
    discore::key commitment;
};

/* all commitments to zero needed for 64-bit amounts :) */
...
EXPORT void GenerateSeckey(key &sk)
{
    random32(sk.bytes);
}

EXPORT key GeneratePubkey()
{
    key sk = skgen();
    key pk = scalarmult_base(sk);
    return pk;
}

EXPORT void GenerateKeypair(key &sk, key &pk)
{
    skgen(sk);
    scalarmult_base(pk, sk);
}

EXPORT void GenCommitment(key &c, const key &a, dis_amount amount)
{
    add_keys2(c, a, d2h(amount), H);
}
 
EXPORT key Commit(dis_amount amount, const key &mask)
{
    key c;
    gen_commitment(c, mask, amount);
    return c;
}

EXPORT key CommitToZero(dis_amount amount)
{
    ...
    key am = d2h(amount);
    key bh = scalarmult_h(am);
    return add_keys(G, bh);
}

EXPORT dis_amount RandomDisAmount(dis_amount limit)
{
    return h2d(skgen()) % limit;
}


EXPORT void ScalarmultBase(key &ag, const key &a)
{
    ge_p3 point;
    sc_reduce32copy(ag.bytes, a.bytes);
    ge_scalarmult_base(&point, ag.bytes);
    ge_p3_tobytes(ag.bytes, &point);
}

EXPORT key ScalarmultBase1(const key &a)
{
    ge_p3 point;
    key ag;
    sc_reduce32copy(ag.bytes, a.bytes);
...
void ScalarmultKey(key &ap, const key &p, const key &a)
{
    ...
    CHECK_THROW_ERR(ge_frombytes_vartime(&_a, p.bytes) == 0, "ge_frombytes_vartime failed (scalarmult_key)");
    ge_scalarmult(&_r, a.bytes, &_a);
    ge_tobytes(ap.bytes, &_r);
}

key ScalarmultKey1(const key &p, const key &a)
{
    ge_p3 _a;
    ge_p2 _r;
...
{
    ge_p2 _r;
    ge_scalarmult(&_r, a.bytes, &ge_p3_H);
    key ap;
    ge_tobytes(ap.bytes, &_r);
    return ap;
}
...
{
    ge_p3 p3;
    return topoint_check_order(&p3, a.bytes);
}

void AddKeys(key &ab, const key &a, const key &b)
{
    ge_p3 b2, a2;
    CHECK_THROW_ERR(ge_frombytes_vartime(&b2, b.bytes) == 0, "ge_frombytes_vartime failed (add_keys)");
    CHECK_THROW_ERR(ge_frombytes_vartime(&a2, a.bytes) == 0, "ge_frombytes_vartime failed (add_keys)");
    ge_cached _tmp2;
    ge_p3_to_cached(&_tmp2, &b2);
    ge_p1p1 _tmp3;
    ge_add(&_tmp3, &a2, &_tmp2);
    ge_p1p1_to_p3(&a2, &_tmp3);
    ge_p3_tobytes(ab.bytes, &a2);
}

key AddKeys_1(const key &a, const key &b)
{
    key k;
    add_keys(k, a, b);
    return k;
...
{
    key ag = scalarmult_base(a);
    add_keys(agb, ag, b);
}

void AddKeys2(key &agbp, const key &a, const key &b, const key &p)
{
    ge_p2 rv;
    ge_p3 p2;
    CHECK_THROW_ERR(ge_frombytes_vartime(&p2, p.bytes) == 0, "ge_frombytes_vartime failed (add_keys2)");
    ge_double_scalarmult_base_vartime(&rv, b.bytes, &p2, a.bytes);
    ge_tobytes(agbp.bytes, &rv);
}

void Precomp(ge_dsmp rv, const key &b)
{
    ge_p3 b2;
    CHECK_THROW_ERR(ge_frombytes_vartime(&b2, b.bytes) == 0, "ge_frombytes_vartime failed (precomp)");
    ge_dsm_precomp(rv, &b2);
}

void AddKeys3(key &apbq, const key &a, const key &p, const key &b, const ge_dsmp q)
{
    ge_p2 rv;
    ge_p3 p2;
    CHECK_THROW_ERR(ge_frombytes_vartime(&p2, p.bytes) == 0, "ge_frombytes_vartime failed (add_keys)");
    ge_double_scalarmult_precomp_vartime(&rv, a.bytes, &p2, b.bytes, q);
    ge_tobytes(apbq.bytes, &rv);
}

void AddKeys3_1(key &apbq, const key &a, const ge_dsmp p, const key &b, const ge_dsmp q)
{
    ge_p2 rv;
    ge_double_scalarmult_precomp_vartime2(&rv, a.bytes, p, b.bytes, q);
    ge_tobytes(apbq.bytes, &rv);
}

void AddKeys4(key &agbpcq, const key &a, const key &b, const ge_dsmp p, const key &c, const ge_dsmp q)
{
    ge_p2 rv;
    ge_triple_scalarmult_base_vartime(&rv, a.bytes, b.bytes, p, c.bytes, q);
    ge_tobytes(agbpcq.bytes, &rv);
}

void AddKeys5(key &apbqcr, const key &a, const ge_dsmp p, const key &b, const ge_dsmp q, const key &c, const ge_dsmp r)
{
    ge_p2 rv;
    ge_triple_scalarmult_precomp_vartime(&rv, a.bytes, p, b.bytes, q, c.bytes, r);
    ge_tobytes(apbqcr.bytes, &rv);
}

void SubKeys(key &ab, const key &a, const key &b)
{
    ge_p3 b2, a2;
    CHECK_THROW_ERR(ge_frombytes_vartime(&b2, b.bytes) == 0, "ge_frombytes_vartime failed (sub_keys)");
    CHECK_THROW_ERR(ge_frombytes_vartime(&a2, a.bytes) == 0, "ge_frombytes_vartime failed (sub_keys)");
    ge_cached _tmp2;
    ge_p3_to_cached(&_tmp2, &b2);
    ge_p1p1 _tmp3;
    ge_sub(&_tmp3, &a2, &_tmp2);
    ge_p1p1_to_p3(&a2, &_tmp3);
    ge_p3_tobytes(ab.bytes, &a2);
}
...
void HashData(key &hash, const void *data, const size_t l)
{
    sha256(hash.bytes, (const unsigned char *)data, l);
}

void HashToScalar(key &hash, const void *data, const size_t l)
{
    hash_data(hash, data, l);
    sc_reduce32(hash.bytes);
}

void HashKey(key &hash, const key &data)
{
    sha256(hash.bytes, (const unsigned char *)data.bytes, 32);
}

void HashKeyToScalar(key &hash, const key &data)
{
    hash_data(hash, data);
    sc_reduce32(hash.bytes);
}

key HashKey1(const key &data)
{
    key hash;
    sha256(hash.bytes, data.bytes, 32);
    return hash;
}

key HashKeyToScalar1(const key &data)
{
    key hash = hash_data(data);
    sc_reduce32(hash.bytes);
    return hash;
}

key HashData128(const void *data)
{
    key hash;
    sha256(hash.bytes, (const unsigned char *)data, 128);
    return hash;
}

key HashToScalar128(const void *data)
{
    key hash = hash_data128(data);
    sc_reduce32(hash.bytes);
    return hash;
}

void HashToP3(ge_p3 &hash8_p3, const key &k)
{
    key hash_key = hash_data(k);
    ge_p2 hash_p2;
...
static key ecdh_hash(const key &k)
{
    char data[38];
    key hash;
    memcpy(data, "amount", 6);
    memcpy(data + 6, &k, sizeof(k));
    hash_data(hash, data, sizeof(data));
    return hash;
}
...
void ECDHEncode(ecdhtuple &unmasked, const key &secret, bool v2)
{
    if (v2) {
        unmasked.mask = zero();
        xor8(unmasked.amount, ecdh_hash(secret));
    }
    else {
        key secret1 = hash_to_scalar(secret);
        key secret2 = hash_to_scalar(secret1);
        sc_add(unmasked.mask.bytes, unmasked.mask.bytes, secret1.bytes);
        sc_add(unmasked.amount.bytes, unmasked.amount.bytes, secret2.bytes);
    }
}

void ECDHDecode(ecdhtuple &masked, const key &secret, bool v2)
{
    if (v2) {
        masked.mask = gen_commitment_mask(secret);
        xor8(masked.amount, ecdh_hash(secret));
    }
    else {
        key secret1 = hash_to_scalar(secret);
        key secret2 = hash_to_scalar(secret1);
