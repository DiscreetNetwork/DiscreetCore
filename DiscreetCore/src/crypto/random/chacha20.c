           ...
           const unsigned char *k)
{
    ...
    int i;
    unsigned char *ctx_ptr = (unsigned char *) &ctx;

    if (!clen) {
        return 0;
    }
    COMPILER_ASSERT(rng_chacha20_KEYBYTES == 256 / 8);
...
int rng_chacha20(unsigned char *c, unsigned long long clen,
                        const unsigned char *n, const unsigned char *k)
{
    if (clen > rng_chacha20_MESSAGEBYTES_MAX) {
        abort();
    }
...
static int rng_chacha20_stream_xor_ic(unsigned char *c, const unsigned char *m,
...
static int rng_chacha20_stream_ietf_xor_ic(unsigned char *c, const unsigned char *m,
                                   unsigned long long mlen,
...
int rng_chacha20_xor(unsigned char *c, const unsigned char *m,
                           ...
                           const unsigned char *k)
{
    if (mlen > rng_chacha20_MESSAGEBYTES_MAX) {
        abort();
    }
    return rng_chacha20_stream_xor_ic(c, m, mlen, n, 0U, k);
}